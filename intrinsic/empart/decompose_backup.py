...
def _process_box(
    ...
    *,
    ...
    if isinstance(im, trimesh.Trimesh):
        im.export("input_manifold_debug.glb")
    else:
        trimesh.Trimesh(
            np.asarray(im.to_mesh().vert_properties),
            np.asarray(im.to_mesh().tri_verts),
            process=False
        ).export(
            "input_manifold_debug.glb"
        )
    ...
    #if watertight:
    #기준 수정 - face 단위로 입력받는 버전
    if "faces" in box:

    # manifold -> trimesh
        # tm_input = trimesh.Trimesh(
        #     np.asarray(im.to_mesh().vert_properties, dtype=np.float32),
        #     np.asarray(im.to_mesh().tri_verts, dtype=np.int64),
        #     process=False
        # )
        ...
        #수정 - mesh 단위로 받는 버전
        tm_input = _INPUT_TRIMESH

        #수정 - face 단위로 받는 버전
        face_ids = box["faces"]

        selected = tm_input.submesh(
            [face_ids],
            append=True,
            repair=False
        )

        verts = selected.vertices.astype(
            np.float32
        )
    ...
    # triangle centers
    #     centers = tm_input.triangles_center
    ...
    #     verts = selected.vertices.astype(np.float32)
    #     faces = np.asarray(
    #         selected.faces,
    #         dtype=np.int32
    #     )
    ...
    flush=True
        ...
        # outputs = _run_global_decomposition(
        #     trimesh.Trimesh(verts, faces, process=False),
        #     decomp_method,
        #     maxConvexHulls,
        #     resolution,
        #     box.get("acdThreshold", 0.001)
        # )
        outputs = _run_global_decomposition(
            trimesh.Trimesh(
                verts,
        ...
        for i, m in enumerate(outputs):
            ...
            f"vhacd_out_{box_index}_{i}.glb"
            )
